  ...
  render(timeMS) {
    ...
    if (!this.params["paused"]) {
      ...
      while (this.mujoco_time < timeMS) {
        ...
        frameCount++;
        if (frameCount % 120 === 0 && this.simulation.xpos && this.simulation.qvel) {
          const droneBodyId = 1; // Adjust if needed
          const pos = { 
            x: this.simulation.xpos[droneBodyId * 3 + 0] || 0,
            y: this.simulation.xpos[droneBodyId * 3 + 1] || 0,
            z: this.simulation.xpos[droneBodyId * 3 + 2] || 2
          };
          const battery = Math.max(0, 100 - (frameCount / 3600) * 100);
          const vel = { 
            x: this.simulation.qvel[droneBodyId * 6 + 0] || 0,
            y: this.simulation.qvel[droneBodyId * 6 + 1] || 0,
            z: this.simulation.qvel[droneBodyId * 6 + 2] || 0
          };
